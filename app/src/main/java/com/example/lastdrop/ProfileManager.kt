...
class ProfileManager(context: Context) {
    ...
    companion object {
        ...
        const val GUEST_NAME = "Guest Player"
    ...
    suspend fun createProfile(name: String, nickname: String = name): Result<PlayerProfile> {
        return try {
            ...
            // Use neutral gray for profile avatar (color chosen at game start)
            val profile = PlayerProfile(
                playerCode = generatePlayerCode(),
                name = name.trim(),
                nickname = nickname.trim(),
                avatarColor = "808080", // Gray - actual game color chosen when playing
                isGuest = false
            )
