/*
 * Last Drop - ESP32 Standalone Mode Firmware (Full Game Logic)
 * 
 * This firmware implements COMPLETE game logic on ESP32 for standalone operation
 * - 20-tile board with tile types (START, NORMAL, CHANCE, BONUS, PENALTY)
 * - 10 chance cards with random selection
 * - Score calculation and tracking
 * - TFT Display with touch interface (ILI9488 480x320)
 * - GoDice BLE integration for smart dice
 * - Virtual dice option via touchscreen
 * - Comprehensive UI with player profiles
 ...
 * Hardware:
 * - ESP32-S3 Dev Board
 * - WS2812B LED Strip (136 LEDs = 4 LEDs per tile × 20 tiles + decorative)
 * - 20 Hall Effect Sensors (A3144) for coin detection
 * - ILI9488 3.5" TFT Display with XPT2046 touch
 * - Optional: SD Card for assets
 */
...
// Set to 1 for standalone operation with TFT display and GoDice
// Set to 0 for Android app controlled operation
#ifndef STANDALONE_BOARD
#define STANDALONE_BOARD 1
#endif

// TFT Display (only for standalone mode)
#if STANDALONE_BOARD
  // TFT_eSPI will be configured via User_Setup.h in library folder
  #define TOUCH_CS_PIN  15  // Touch controller chip select
#endif
...
// ==================== BLE CONFIGURATION ====================
// Android App BLE Service (for non-standalone mode)
#define SERVICE_UUID        "6e400001-b5a3-f393-e0a9-e50e24dcca9e"
#define CHARACTERISTIC_UUID_RX "6e400002-b5a3-f393-e0a9-e50e24dcca9e"
#define CHARACTERISTIC_UUID_TX "6e400003-b5a3-f393-e0a9-e50e24dcca9e"

// GoDice BLE Service (for standalone mode)
...
#define GODICE_CMD_BATTERY     0x03
#define GODICE_CMD_GET_COLOR   0x17
#define GODICE_CMD_SET_LED     0x08
#define GODICE_CMD_PULSE_LED   0x10

// GoDice Protocol Messages
#define GODICE_MSG_ROLLING     0x52  // 'R'
#define GODICE_MSG_STABLE      0x53  // 'S'
#define GODICE_MSG_FAKE_STABLE 0x46  // 'F'
#define GODICE_MSG_TILT_STABLE 0x54  // 'T'
#define GODICE_MSG_MOVE_STABLE 0x4D  // 'M'
#define GODICE_MSG_BATTERY     0x42  // 'B'
#define GODICE_MSG_COLOR       0x43  // 'C'

// GoDice XYZ vectors for face detection
const int8_t GODICE_D6_VECTORS[6][3] = {
...
// Note: STANDALONE_BOARD is defined as macro above (line 41)
// #define STANDALONE_BOARD 1
...
// Android App BLE (for non-standalone mode)
BLEServer *pServer = NULL;
BLECharacteristic *pTxCharacteristic;
bool deviceConnected = false;
bool oldDeviceConnected = false;

// GoDice BLE Client (for standalone mode)
#include <BLEScan.h>
#include <BLEAdvertisedDevice.h>

BLEScan* pGoDiceScan = nullptr;
BLEClient* pGoDiceClient = nullptr;
BLERemoteCharacteristic* pGoDiceTxChar = nullptr;
BLERemoteCharacteristic* pGoDiceRxChar = nullptr;

bool goDiceConnected = false;
bool goDiceScanning = false;
String goDiceName = "";
int lastDiceValue = 0;
...
// GoDice mode function (only used when STANDALONE_BOARD == true)
...
#if STANDALONE_BOARD
// Display manager must come first (provides displayState, profiles, etc.)
#include "display_manager.h"
// GoDice integration uses display functions
#include "godice_integration.h"
#endif
...
void setup() {
  ...
  // Show logo screen
  changeScreen(SCREEN_LOGO);
  Serial.println("✓ Display showing logo screen");
#endif
...
void initBLE() {
#if STANDALONE_BOARD
  // Standalone mode: Init as BLE client for GoDice
  Serial.println("🎮 STANDALONE MODE - Connecting to GoDice");
  BLEDevice::init("LastDrop-Standalone");
  
  Serial.print("ESP32 MAC Address: ");
  Serial.println(BLEDevice::getAddress().toString().c_str());
  
  // Set up BLE scan
  pGoDiceScan = BLEDevice::getScan();
  pGoDiceScan->setAdvertisedDeviceCallbacks(new GoDiceScanCallbacks());
  
  // Start scanning for GoDice
  startGoDiceScan();
  
  // Set connection mode to disconnected (blue corner LEDs)
  currentConnectionMode = MODE_DISCONNECTED;
  Serial.println("✓ BLE client initialized - corner LEDs will blink blue until dice connects\n");
  return;  // Skip server setup
#endif

  // Android mode: Init as BLE server
...
#if STANDALONE_BOARD
  // Standalone mode: Handle GoDice connection
  handleGoDiceConnection();
  
  // Update TFT display (touch handling and animations)
  updateDisplay();
  
  // Update connection status LEDs (blue blink until dice connects)
  if (!goDiceConnected && !waitingForCoin && displayState.currentScreen == SCREEN_DICE_CONNECT) {
    updateConnectionStatusLEDs();
  }
  
  // If dice connected, update display
  if (goDiceConnected && displayState.currentScreen == SCREEN_DICE_CONNECT) {
    drawDiceConnectScreen("Connected!", true);
    delay(1000);
    changeScreen(SCREEN_GAMEPLAY);
    currentConnectionMode = MODE_READY;
    Serial.println("✅ GoDice connected - starting game!");
  }
#else
  // Android mode: BLE connection handling
