 ...
 */
bool BLEClient::connect(BLEAddress address, uint8_t type, uint32_t timeoutMs) {
  Serial.printf("[BLEClient] >> connect(%s) type=%d timeout=%d\n", address.toString().c_str(), type, timeoutMs);

  if (!BLEDevice::m_synced) {
    Serial.println("[BLEClient] ERROR: Host not synced, wait for sync.");
    return false;
  }

  if (m_conn_id != BLE_HS_CONN_HANDLE_NONE || m_isConnected || m_pTaskData != nullptr) {
    Serial.printf("[BLEClient] ERROR: Client busy, connected to %s, id=%d\n", m_peerAddress.toString().c_str(), getConnId());
    return false;
  }

  // Build peer address structure
  ble_addr_t peerAddr_t;
  memcpy(&peerAddr_t.val, address.getNative(), 6);
  
  // CRITICAL FIX: Use the type parameter passed to this function
  // The 'type' parameter comes from the scan result and is the correct address type
  // GoDice uses BLE_ADDR_RANDOM (1) for its random static address
  peerAddr_t.type = type;  // Use parameter directly!
  
  Serial.printf("[BLEClient] Peer address type: %d (0=PUBLIC, 1=RANDOM)\n", peerAddr_t.type);
  Serial.printf("[BLEClient] Peer address: %02x:%02x:%02x:%02x:%02x:%02x\n",
    peerAddr_t.val[5], peerAddr_t.val[4], peerAddr_t.val[3],
    peerAddr_t.val[2], peerAddr_t.val[1], peerAddr_t.val[0]);
  Serial.printf("[BLEClient] Own address type: %d\n", BLEDevice::m_ownAddrType);
  
  if (ble_gap_conn_find_by_addr(&peerAddr_t, NULL) == 0) {
    Serial.printf("[BLEClient] ERROR: Connection to %s already exists\n", address.toString().c_str());
    return false;
  }

  if (address == BLEAddress("")) {
    Serial.println("[BLEClient] ERROR: Invalid peer address (NULL)");
    return false;
  }
  ...
  do {
    attempts++;
    Serial.printf("[BLEClient] Connection attempt %d/%d...\n", attempts, maxAttempts);
    
    rc = ble_gap_connect(BLEDevice::m_ownAddrType, &peerAddr_t, m_connectTimeout, &m_pConnParams, BLEClient::handleGAPEvent, this);
    
    Serial.printf("[BLEClient] ble_gap_connect returned: %d\n", rc);
    
    switch (rc) {
      case 0: 
        Serial.println("[BLEClient] Connection initiated successfully!");
        break;

      case BLE_HS_EBUSY:
        Serial.println("[BLEClient] WARN: BLE_HS_EBUSY - Scan still running, stopping...");
        // Scan was still running, stop it and try again
        if (!BLEDevice::getScan()->stop()) {
          rc = BLE_HS_EUNKNOWN;
        }
        vTaskDelay(100 / portTICK_PERIOD_MS);  // Small delay after stopping scan
        break;

      case BLE_HS_EDONE:
        Serial.printf("[BLEClient] ERROR: BLE_HS_EDONE - Already connected to %s\n", m_peerAddress.toString().c_str());
        break;

      case BLE_HS_EALREADY:
        Serial.printf("[BLEClient] ERROR: BLE_HS_EALREADY - Already attempting connect to %s\n", m_peerAddress.toString().c_str());
        ble_gap_conn_cancel();
        vTaskDelay(500 / portTICK_PERIOD_MS);  // Wait for cancel
        break;

      default: 
        Serial.printf("[BLEClient] ERROR: ble_gap_connect failed rc=%d; %s\n", rc, BLEUtils::returnCodeToString(rc));
        if (attempts < maxAttempts) {
          Serial.println("[BLEClient] Waiting 1 second before retry...");
          vTaskDelay(1000 / portTICK_PERIOD_MS);
        }
        break;
    }

  } while ((rc == BLE_HS_EBUSY || (rc != 0 && attempts < maxAttempts)));

  if (rc != 0) {
    Serial.printf("[BLEClient] All connection attempts failed, last error: %d\n", rc);
    m_lastErr = rc;
    return false;
  }

  Serial.printf("[BLEClient] Waiting for connection (timeout: %d ms)...\n", m_connectTimeout + 1000);
...
int BLEClient::handleGAPEvent(struct ble_gap_event *event, void *arg) {
  ...
  Serial.printf("[BLEClient] GAP Event: %d (%s)\n", event->type, BLEUtils::gapEventToString(event->type));
  ...
  switch (event->type) {
    ...
    {
      Serial.printf("[BLEClient] BLE_GAP_EVENT_CONNECT received! status=%d\n", event->connect.status);
      ...
      if (client->isConnected() || client->m_pTaskData == nullptr) {
        Serial.println("[BLEClient] WARNING: Already connected or no task data - terminating!");
      ...
      if (rc == 0) {
        Serial.printf("[BLEClient] Connection SUCCESS! Handle: %d\n", event->connect.conn_handle);
        log_i("BLEClient: Connected event. Handle: %d", event->connect.conn_handle);

        client->m_conn_id = event->connect.conn_handle;
        client->m_isConnected = true;  // Mark as connected BEFORE MTU exchange

        rc = ble_gattc_exchange_mtu(client->m_conn_id, nullptr, nullptr);
        if (rc != 0 && rc != BLE_HS_EALREADY) {
          // Only log, don't fail - MTU exchange is optional
          Serial.printf("[BLEClient] MTU exchange returned: %d (continuing anyway)\n", rc);
          log_e("BLEClient", "MTU exchange error; rc=%d %s", rc, BLEUtils::returnCodeToString(rc));
        }

        // Release the waiting task to signal connection success
        if (pTaskData != nullptr) {
          Serial.println("[BLEClient] Releasing task - connection complete!");
        ...
        Serial.printf("[BLEClient] Connection FAILED with status: %d\n", rc);
        client->m_conn_id = BLE_HS_CONN_HANDLE_NONE;
        if (pTaskData != nullptr) {
